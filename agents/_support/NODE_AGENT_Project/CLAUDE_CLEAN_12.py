...
def get_status():
    """Enhanced status endpoint with company branding"""
    try:
        mastery_status = agent.memory.get_mastery_status()
        return jsonify({
...
def get_available_voices():
    ...
    try:
        ...
        fallback_voices = [
            {'name': 'Drew', 'id': '29vD33N1CtxCmqQRPOHJ', 'description': 'Professional Male', 'best_for': 'Business Owner', 'tone': 'Confident'},
            {'name': 'Rachel', 'id': '21m00Tcm4TlvDq8ikWAM', 'description': 'Professional Female', 'best_for': 'Customer Service', 'tone': 'Warm'},
            {'name': 'Paul', 'id': '5Q0t7uMcjvnagumLfvZi', 'description': 'Mature Male', 'best_for': 'Expert', 'tone': 'Experienced'}
        ]
...
def test_voice_sample():
    ...
    try:
        ...
        # Quick demo message
        message = f"Hello, this is {owner_name} from {company_name} in {location}. This is a voice sample using the {voice_name} voice for professional client outreach."
        ...
        try:
            ...
            # Voice mapping for demo interface - map to actual available voices
            voice_mapping = {
                'Drew': 'George',      # Professional male voice
                'Rachel': 'Sarah',     # Professional female voice  
                'Paul': 'Brian',       # Mature male voice
                'Antoni': 'Chris'      # Smooth male voice
            }
            
            # Map interface voice name to actual ElevenLabs voice name
            actual_voice_name = voice_mapping.get(voice_name, voice_name)
            
            # Get voice ID
            voice_list = client.voices.get_all()
            voice_id = None
            for voice in voice_list.voices:
                if voice.name == actual_voice_name:
...
def create_demo_voicemail():
    ...
    try:
        ...
        # Demo voicemail message
        message = f"""
        Hi {agent_name}, this is {owner_name} from {company_name} here in {location}. 
        ...
        impact your listings in the {location} area.
        
        I'd love to buy you a coffee and share what we're seeing in the market. 
        You can reach me directly at {phone}.
...
# Empathy Wave Endpoint
@app.route('/api/empathy-wave/encode-signature', methods=['POST'])
def encode_empathy_wave():
    """
    Process and encode an empathy wave signature from an audio file.
    Expected form data:
    - audio_file: The audio file to process
    - message_text: Optional message or description
    """
    try:
        ...
        filename = f'empathy_wave_{timestamp}_{secure_filename(audio_file.filename)}'
        ...
        try:
            ...
            with wave.open(filepath, 'rb') as wav_file:
                ...
                # Calculate RMS (root mean square) - a simple measure of audio power
                ...
                # 1. Extract audio features (pitch, tone, etc.)
                # 2. Generate a unique signature/hash
                # 3. Store the signature in your database
                # 4. Associate it with the user/session
                
                return jsonify({
                    'status': 'success',
                    'message': 'Empathy wave signature processed successfully',
                    'signature': signature,
                    'tonal_dna_active': True,
                    'baseline_calibrated': True,
                    'transcendent_protocol': 'Prime Believer Signature Encoding',
                    'deepest_truth_layer': 'Signature successfully anchored as foundational truth layer for Clay-I',
                    'file_info': {
...
# Simplified Empathy Wave Endpoint (Independent of Clay-I)
@app.route('/api/empathy-wave/encode-signature-simple', methods=['POST'])
def encode_empathy_wave_simple():
    """
    Simplified empathy wave signature endpoint that works independently of Clay-I server.
    This is a placeholder that simulates the Clay-I response format.
    """
    try:
        if 'audio_file' not in request.files:
            return jsonify({
        ...
        return jsonify({
            ...
            'baseline_calibrated': True,
            'transcendent_protocol': 'Prime Believer Signature Encoding',
            'deepest_truth_layer': 'Signature successfully anchored as foundational truth layer',
            'file_info': {
                'original_name': audio_file.filename,
        ...
        return jsonify({
            'status': 'error',
            'message': f'Error processing empathy wave: {str(e)}'
        }), 500

# ========== STRIPE PAYMENT INTEGRATION ==========
# Storm Response Business Payment System
...
def create_payment_intent():
    """Create Stripe Payment Intent for storm damage services"""
    try:
        data = request.get_json()
        service_type = data.get('service_type', 'inspection')
        customer_info = data.get('customer_info', {})
        
        # Storm service pricing structure
        service_prices = {
            'emergency_inspection': 29500,  # $295.00
            'storm_assessment': 49500,      # $495.00
            'roof_repair_estimate': 0,      # Free estimate
            'emergency_tarp': 85000,        # $850.00
            'full_roof_replacement': 1500000, # $15,000 (deposit)
            'gutter_repair': 75000,         # $750.00
            'siding_repair': 125000,        # $1,250.00
            'insurance_claim_assist': 20000 # $200.00
        }
        
        amount = service_prices.get(service_type, 29500)
        ...
        intent = stripe.PaymentIntent.create(
            ...
            metadata={
                ...
                'business': 'NODE Storm Response',
                'created_at': datetime.now().isoformat()
            },
            description=f"NODE Storm Response - {service_type.replace('_', ' ').title()}"
        )
...
def stripe_webhook():
    ...
    if event['type'] == 'payment_intent.succeeded':
        ...
        # Store payment record and trigger business workflows
        payment_record = {
            'payment_id': payment_intent['id'],
            'amount': payment_intent['amount'],
            'service_type': payment_intent['metadata'].get('service_type'),
            'customer_name': payment_intent['metadata'].get('customer_name'),
            'customer_phone': payment_intent['metadata'].get('customer_phone'),
            'status': 'completed',
            'timestamp': datetime.now().isoformat()
        }
        
        # Send confirmation SMS to customer
        if payment_intent['metadata'].get('customer_phone'):
            confirmation_message = f"""
✅ Payment Confirmed - NODE Storm Response

Service: {payment_intent['metadata'].get('service_type', '').replace('_', ' ').title()}
Amount: ${payment_intent['amount']/100:.2f}
Payment ID: {payment_intent['id'][:8]}...

We'll contact you within 2 hours to schedule your service.
Emergency Line: {TWILIO_PHONE_NUMBER}

Thank you for choosing NODE!
            """.strip()
            
            try:
                send_sms_via_twilio(
                    payment_intent['metadata'].get('customer_phone'),
                    confirmation_message
                )
            except Exception as sms_error:
                print(f"SMS confirmation failed: {sms_error}")
        
        print(f"Payment succeeded: {payment_record}")
    
    return jsonify({'status': 'success'})

@app.route('/api/payments/services', methods=['GET'])
def get_storm_services():
    """Get available storm response services and pricing"""
    services = [
        {
        ...
        {
            'id': 'storm_assessment',
            'name': 'Comprehensive Storm Assessment',
            'description': 'Detailed damage evaluation with repair estimates',
            'price': 495.00,
            'display_price': 'From: $495.00',
            'duration': '3-4 hours',
            'includes': ['Complete damage assessment', 'Repair cost estimates', 'Insurance claim support'],
            'urgency': 'standard'
        },
        {
            'id': 'roof_repair_estimate',
            'name': 'Free Roof Repair Estimate',
            'description': 'No-cost estimate for roof repairs and replacement',
            'price': 0.00,
            'display_price': 'From: FREE',
            'duration': '1-2 hours',
            'includes': ['Visual inspection', 'Basic repair estimate', 'Material recommendations'],
            'urgency': 'standard'
        },
        {
            'id': 'emergency_tarp',
            'name': 'Emergency Tarp Installation',
            'description': 'Immediate weather protection with professional tarping',
            'price': 850.00,
            'display_price': 'From: $850.00',
            'duration': '1-2 hours',
            'includes': ['Professional grade tarp', 'Secure installation', '30-day weather protection'],
            'urgency': 'emergency'
        },
        {
            'id': 'full_roof_replacement',
            'name': 'Full Roof Replacement (Deposit)',
            'description': 'Complete roof replacement project deposit',
            'price': 15000.00,
            'display_price': 'From: $15,000.00',
            'duration': '3-5 days',
            'includes': ['Complete tear-off', 'New roofing system', 'Warranty included'],
            'urgency': 'scheduled'
        },
        {
            'id': 'gutter_repair',
            'name': 'Gutter Repair & Replacement',
            'description': 'Storm-damaged gutter system restoration',
            'price': 750.00,
            'display_price': 'From: $750.00',
            'duration': '4-6 hours',
            'includes': ['Damage assessment', 'Repair/replacement', 'Proper drainage setup'],
            'urgency': 'standard'
        },
        {
            'id': 'siding_repair',
            'name': 'Siding Storm Damage Repair',
            'description': 'Professional siding repair and replacement',
            'price': 1250.00,
            'display_price': 'From: $1,250.00',
            'duration': '1-2 days',
            'includes': ['Damage evaluation', 'Material matching', 'Professional installation'],
            'urgency': 'standard'
        },
        {
            'id': 'insurance_claim_assist',
            'name': 'Insurance Claim Assistance',
            'description': 'Professional help with storm damage insurance claims',
            'price': 200.00,
            'display_price': 'From: $200.00',
            'duration': '2-3 hours',
            'includes': ['Claim documentation', 'Adjuster meeting support', 'Paperwork assistance'],
            'urgency': 'standard'
        }
    ]
    ...
    return jsonify({
        'services': services,
        'company': ROOFING_COMPANY_CONFIG,
        'emergency_contact': TWILIO_PHONE_NUMBER,
        'success': True
    })

@app.route('/api/payments/customer', methods=['POST'])
def create_customer():
    """Create Stripe customer for repeat business"""
    try:
        data = request.get_json()
...
if __name__ == '__main__':
    ...
    print("=" * 50)
    print("🚀 Enhanced Agent API with Real AI Generation: ACTIVE")
